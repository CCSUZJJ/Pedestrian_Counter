...
void TiXmlBase::EncodeString( const TIXML_CPP_STRING& str, TIXML_CPP_STRING* outString )
{
	int i=0;
...
{
    #ifdef TIXML_CPP_USE_STL
    TIXML_CPP_STRING _name( cname );
    TIXML_CPP_STRING _value( cvalue );
	#else
...
bool TiXmlDocument::LoadFile( const char* _filename, TiXmlEncoding encoding )
{
    ...
    TIXML_CPP_STRING filename( _filename );
	value = filename;
...
void TiXmlAttribute::Print( FILE* cfile, int /*depth*/, TIXML_CPP_STRING* str ) const
{
    TIXML_CPP_STRING n, v;

	EncodeString( name, &n );
	EncodeString( value, &v );

    if (value.find ('\"') == TIXML_CPP_STRING::npos) {
		if ( cfile ) {
...
void TiXmlDeclaration::Print( FILE* cfile, int /*depth*/, TIXML_CPP_STRING* str ) const
{
...
void TiXmlStylesheetReference::Print( FILE* cfile, int /*depth*/, TIXML_CPP_STRING* str ) const
{
...
{
    #ifdef TIXML_CPP_USE_STL
    assert( !Find( TIXML_CPP_STRING( addMe->Name() ) ) );	// Shouldn't be multiply adding to the set.
	#else
