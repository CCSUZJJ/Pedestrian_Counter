...
void DeferredTestReporter::ReportBenchmark(const TestDetails &details, const std::vector<float> &Durations)
{
    DeferredTestResult& r = m_results.back();
    r.benchmarked = true;
    r.benchmark = DeferredTestResult::Benchmark(details.lineNumber, Durations);
    r.benchmarkFile = details.filename;
}
