...
void XmlTestReporter::AddBenchmark(std::ostream &os, const DeferredTestResult &result)
{
    os << ">"; // close <test> element

    ostringstream benchmarkMessage;
    benchmarkMessage << "repeats=" << result.benchmark.second.size()
                     << " durations=";
    for(auto duration : result.benchmark.second)
    {
        benchmarkMessage << duration << " ";
    }

    string const message = BuildFailureMessage(result.benchmarkFile, result.benchmark.first, benchmarkMessage.str());

    os << "<benchmark" << " message=\"" << message << "\"" << "/>";
}
